...
public class P15662 {
    ...
    static void input(String[] lines) {
        ...
        gears = new Gear[T];
        for (int i = 0; i < T; i++) {
            int[] input = strToIntArray(lines[i + 1]);
        ...
        K = Integer.parseInt(lines[T + 1]);
        rotateList = new ArrayList<>();
        for (int i = 0; i < K; i++) {
            StringTokenizer st = new StringTokenizer(lines[T + 2 + i]);
    ...
    static String process() {
        ...
        Arrays.stream(gears).filter(gear -> gear.getPole("top") == 1).forEach(gear -> result++);
